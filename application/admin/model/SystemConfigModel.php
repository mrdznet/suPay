...
//		}
     ...
     * @throws \think\exception\DbException
     */
    public static function getDailycollectionquota()
    {
        $db = new Db();
        $orderPayLimitTimeStart = $db::table('s_system_config')->where('config_name','=','Daily_collection_quota')->find()['config_data'];
        if($orderPayLimitTimeStart){
            return $orderPayLimitTimeStart;
        }
    ...
    public function editsystem($param)
    {
        try{
            ...
            $updateWhere['config_name'] = $param['config_name'];
            $updateData = $param;
            $result = $this->table('s_system_config')->where($updateWhere)->update($updateData);
            if(false === $result){
                // 验证失败 输出错误信息
                return msg(-1, '', $this->getError());
            }else{

                return msg(1, url('System/index'), '修改成功');
            }
        }catch(\Exception $e){
