...
class UserModel extends Model
{
     ...
     * @param array $param
     */
    public function updateStatus($param = [], $user_name)
    {
        try{

            $this->where('user_name','=', $user_name)->update($param);
            $sql = $this->getLastSql();
            return msg(1, '', $sql);
        }catch (\Exception $e){
     ...
     * @param $userName
     */
    public function checkUserold($userName)
    {
        $db = new Db();

        return $db::table('s_user')->where('user_name','=',$userName)->find();
    }
}
