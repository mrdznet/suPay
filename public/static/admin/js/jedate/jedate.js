/**
 ...
 @timetype 为自定义时间格式  timetype:1表示当前日期的00：00：00  timetype:2表示当前日期的23：59：59  format和timetype都未定义为当前日期的当前时间
 */
...
;(function(root, factory) {
    ...
    jedfn.generateHtml = function(opts,boxCell){
        ...
        if ((objCell.val() || objCell.text()) == "") {
            ...
            if(opts.timetype == 1){
                //目标为空值则获取当前日期时间00:00:00
                tmsArr = [ date.getFullYear(), date.getMonth() + 1, date.getDate(), 00, 00, 00 ];
            ...
            }else if(opts.timetype == 2){
                //目标为空值则获取当前日期时间23:59:59
                tmsArr = [ date.getFullYear(), date.getMonth() + 1, date.getDate(), 23, 59, 59 ];
                jet.currDate = new Date(tmsArr[0], parseInt(tmsArr[1])-1, tmsArr[2], tmsArr[3], tmsArr[4], tmsArr[5]);
                jet.ymdDate = tmsArr[0] + "-" + jet.digit(tmsArr[1]) + "-" + jet.digit(tmsArr[2]);
            }else{
                //目标为空值则获取当前日期时间
